function slider({container, slide, nexArrow, prevArrow, totalCounter, currentCounter, wrapper, field}) {
    ...
    function WithReplace(str) {
        return +str.replace(/\D/g, '');
    }

    next.addEventListener('click', () => {
        if (offset == WithReplace(width) * (slides.length - 1)) {
            offset = 0;
        } else {
            offset += WithReplace(width);
        }
    ...
    prev.addEventListener('click', () => {
        if (offset == 0) {
            offset = WithReplace(width) * (slides.length - 1);
        } else {
            offset -= WithReplace(width);
        }
    ...
    dots.forEach(dot => {
        dot.addEventListener('click', (e) => {
            ...
            slideIndex = slideTo;
            offset = WithReplace(width) * (slideTo - 1);
